...
- **`POST /api/projects/:pid/nodes/:nid/suggestions` returned 500 "Failed to queue suggestion"** when the agent didn't supply `evidence`. `repository/suggestion.go evidenceBytes(nil)` mapped to SQL NULL, violating `node_suggestions.evidence JSONB NOT NULL DEFAULT '{}'` (migration 009). Fix: return `[]byte("{}")` so the schema default is honoured.
- **Silent audit-log drop on every mutation.** `repository/audit.go marshalAuditMetadata(nil)` returned `nil` → SQL NULL → `audit_log.metadata JSONB NOT NULL` constraint violation. `audit.Logger.Log()` only `slog.Warn`s on insert failure (deliberately non-blocking), so the row was lost without surfacing — node/edge `create/update/delete` traced as zero rows in `audit_log` per request despite the Phase 11 "every write logs one row" invariant. Fix: return `json.RawMessage("{}")` for nil / empty / marshal-error. Verified across all five mutation classes via `SELECT COUNT(*)` before/after delta on the live DB.
- **Cross-project edge endpoints no longer silently corrupt the graph.** Authenticated callers with multi-project access could `POST /edges` (single) or `POST /edges/batch-create` (bulk) passing a `sourceId` or `targetId` from a different project than the request's `:projectId` path param; the row landed with `project_id` = path arg but its `source_id` / `target_id` referenced nodes in another project, producing dangling cross-project rows that broke `FindByProjectID` joins. New sqlc queries `EdgeVerifyNodesInProject` (single) + `EdgeListNodeIDsInProject` (batch) plus `EdgeRepo.VerifyNodesInProject` / `FilterNodesInProject` wrappers gate both handlers. Single Create returns `400 "Source and target nodes must both belong to this project"`. Bulk BatchCreate routes mismatched items to `skipped[]` with `reason="cross_project"` and continues honouring the 207 contract for the valid remainder.
...
- **New `docs/SCENARIOS.md` — 295-scenario catalog** covering every observable Thask behavior, derived directly from `routes.go` (69 backend endpoints), `cli/internal/cmd/*.go` (47 CLI subcommands), `cli/internal/mcp/tools.go` (24 MCP tools), and `frontend/src/routes/**/+page.svelte` (13 pages). Each scenario has an ID (AUTH-01, NODE-22, ERR-15, etc.) so E2E runs, PR reviews, and bug reports can cite precisely. Section §16 documents how to use it for E2E selection, regression sweeps, CHANGELOG bullets, and the per-release audit checklist.
- **PRD-driven E2E verification (ralph round, 36/36 live assertions PASS).** `.omc/prd.json` defined 8 user stories with task-specific acceptance criteria (permission gates, audit invariant, suggestion queue E2E, bulk 207 semantics, MCP dispatch, CLI domain commands, error edges, architect review). All eight marked `passes:true` against fresh evidence; logs persisted at `/tmp/e2e-{perm,audit,sugg,bulk,mcp,cli,edges}.log` and `/tmp/e2e-architect.md`. Bug fixes above (`evidenceBytes`, `marshalAuditMetadata`, edge cross-project single + batch, BatchCreate waypoints NULL, CLI sniffer expansion) were all surfaced and closed inside this loop.
- **Recurring pattern catalogued for future memory: `xxxBytes(nil) → nil → SQL NULL → NOT NULL JSONB violation`.** This shape appeared in four separate locations this session (suggestion.evidence, audit_log.metadata, edges.waypoints inline, generic) and silently swallows writes when the call site's error handler is `slog.Warn`-only. Future schema columns that ship as `JSONB NOT NULL DEFAULT '...'` must have a matching `nil`-aware marshaller before they're wired into a hot path.
